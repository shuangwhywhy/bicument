...
if (fs.existsSync(`${CWD}/package.json`)) {
  ...
  packageContent.scripts.start = 'bic-start';
  packageContent.scripts.build = 'bic-build';
  packageContent.scripts['publish-gh-pages'] = 'bic-publish';
  packageContent.scripts.examples = 'bic-examples';
  packageContent.scripts['write-translations'] =
    'bic-write-translations';
  packageContent.scripts.version = 'bic-version';
  packageContent.scripts['rename-version'] = 'bic-rename-version';
  fs.writeFileSync(
    `${CWD}/package.json`,
    `${JSON.stringify(packageContent, null, 2)}\n`,
  );
  console.log(
    `${chalk.green('Wrote bicument scripts to package.json file.')}\n`,
  );
}
  ...
  if (fs.existsSync(targetDocsDir)) {
    console.log(
      `- ${chalk.green('docs')} already exists in ${chalk.blue(
        outerFolder,
      )}. Copying into ${CWD}/../docs-examples-from-bicument instead.`,
    );
    targetDocsDir = `${CWD}/../docs-examples-from-bicument`;
...
if (!docsCreatedInIntendedDirectory) {
  console.log(
    `The ${chalk.yellow(
      ...
      `Please manually convert the contents into a bicument-compatible format ` +
      `by referring to the examples from ${chalk.yellow(
        `${outerFolder}/docs-examples-from-bicument`,
      )}.\n`,
  );
}
