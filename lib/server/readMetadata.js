...
function readSidebar(sidebars = {}) {
  ...
  Object.keys(sidebars).forEach(sidebar => {
    ...
    for (let i = 0; i < sidebarItems.length; i++) {
      ...
      // by qyz:
      if ('directLink' in item) {
        items[item.id].directLink = item.directLink;
      }
      if ('level' in item) {
...
function processMetadata(file, refDir) {
  ...
  // Example:
  //  (file: 'bicument/docs/projectA/test.md', ID 'test', refDir: 'bicument/docs')
  ...
  if (item) {
    ...
    // by qyz:
    if ('directLink' in item) {
      metadata.directLink = item.directLink;
    }
    if ('level' in item) {
  ...
  // extract the headers add to metadata by alexq
  const { extractHeaders, parseFrontmatter } = require("@vuepress/shared-utils");
  const createMarkdown = require('@vuepress/markdown');
  
  const { content: strippedContent } = parseFrontmatter(rawContent);
...
function generateMetadataDocs() {
  ...
  versionData.forEach(metadata => {
    ...
    if (order[id]) {
      ...
      // by qyz:
      if ('directLink' in order[id]) {
        metadata.directLink = order[id].directLink;
      }
      if ('level' in order[id]) {
        metadata.level = order[id].level;
      }
  ...
  // 
  const arr = Object.keys(metadatas).map((k) => metadatas[k]);
  fs.writeFileSync(
    path.join(__dirname, '/../static/js/doc_metadata.js'),
    `${'/**\n' +
    ' * @' +
    'generated automatically\n' + // separate this out for Nuclide treating @generated as readonly
      ' */\n' +
      'window.pageData = '}${JSON.stringify(arr.map(({ title, headers, permalink: path }) => ({ title, headers, path })))};\n`,
  );
