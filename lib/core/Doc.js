...
class Doc extends React.Component {
  renderContent() {
    ...
    return content.replace(
      /(\t|\s{4})*?(<!--BICUMENT_CODE_TABS-->\n)(.*?)((\n|\t|\s{4})<!--END_BICUMENT_CODE_TABS-->)/gms,
      m => {
        const contents = m.split('\n').filter(c => {
          if (!indents) {
            indents = (
              c.match(/((\t|\s{4})+)<!--BICUMENT_CODE_TABS-->/) || []
            ).length;
          }
          if (c.match(/(\t|\s{4})+<!--BICUMENT_CODE_TABS-->/)) {
            return false;
          }
          if (
            c.match(
              /<!--END_BICUMENT_CODE_TABS-->|<!--BICUMENT_CODE_TABS-->/,
            ) ||
            (indents > 0 &&
              c.match(
                /(\t|\s{4})+(<!--END_BICUMENT_CODE_TABS-->|<!--BICUMENT_CODE_TABS-->)/,
              ))
          ) {
            return false;
          }
