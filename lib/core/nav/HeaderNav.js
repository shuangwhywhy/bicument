...
const UsageTips = require('./UsageTips.js');
...
// header navbar used by all pages generated with bicument
class HeaderNav extends React.Component {
  ...
  componentDidMount () {
    this.setState(state => {
      state.pageData = window.pageData;
      return state;
    });
  }
      ...
      if (
        ...
        throw new Error(
          `You have 'doc' in your headerLinks, but no '${readMetadata.getDocsPath()}' folder exists one level up from ` +
            `'website' folder. Did you run \`bicument-init\` or \`npm run examples\`? If so, ` +
            `make sure you rename 'docs-examples-from-bicument' to 'docs'.`,
        );
      }
      if (link.blog && !fs.existsSync(`${CWD}/blog/`)) {
        throw new Error(
          "You have 'blog' in your headerLinks, but no 'blog' folder exists in your " +
            "'website' folder. Did you run `bicument-init` or `npm run examples`? If so, " +
            "make sure you rename 'blog-examples-from-bicument' to 'blog'.",
        );
      }
