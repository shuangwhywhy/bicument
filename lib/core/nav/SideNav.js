...
class SideNav extends React.Component {
  ...
  renderCategory = categoryItem => {
    ...
    if (categoryItem.item && categoryItem.item.directLink) {
      // if-block by qyz:
      const linkMetadata = categoryItem.item;
      const itemClasses = classNames('navGroupCategoryTitle', {
        navListItemActive: linkMetadata.id === this.props.current.id,
      });
      switch (categoryItem.item.level) {
        case 1:
          break;
        case 2:
          break;
      ...
      return (
        <div className="navGroup" key={linkMetadata.id}>
          <h3 className={itemClasses}>
